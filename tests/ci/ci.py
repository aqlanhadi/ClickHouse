...
from ci_config import CI_CONFIG, Labels, JobNames
...
class CiCache:
    """
    CI cache records are files in s3 bucket with names as follows:
        job_<JOB_NAME>_<JOB_DIGEST>_<BATCH>_<NUM_BATCHES>.ci
    Record types (file extensions):
        .ci - for successfully finished job, content is CommitStatusData
        .master.ci - for successfully finished job on the master, content is CommitStatusData
        .cip - pending jobs. Only maser job can be pending by design. Jobs in other CI runs can await on them. Content is PendingStatus (not used currently)
    """
    ...
    def _get_job_status_file_name_and_s3path(
        ...
        """generates filename and s3 path for a cache record"""
        file_name = (
    ...
    def fetch_job_statuses(self):
        ...
        files = self.s3.download_files(  # type: ignore
        ...
        print(f"CI docs metadata files [{files}]")
        return self
    ...
    def is_successful(
        ...
        if not self.cache_updated:
            self.update()
        file_name, _ = self._get_job_status_file_name_and_s3path(
            job, batch, num_batches
        )  # type: Tuple[str, str]
        cache_list = (
            self.successful_jobs if not on_master else self.successful_on_master_jobs
        )
        print(f"job [{job}] list [{cache_list}], [{file_name in cache_list}]")
        return file_name in cache_list
    ...
    def pop(self, job: str, batch: int, num_batches: int) -> Optional[CommitStatusData]:
        """
        Pops a cache record (CommitStatusData) for a job, or None if a cache miss
        """
        if not self.cache_path.exists():
            print(
        ...
        success_flag_name, _ = self._get_job_status_file_name_and_s3path(
            job, batch, num_batches
        )
        print(f"Pop [{success_flag_name}]")
...
def _configure_jobs(
    ...
    for job in digests:
        ...
        for batch in range(num_batches):  # type: ignore
            if job_config.disabled_on_master and PRInfo().number:
                continue
    ...
    print(f"Intermediate jobs_to_do: [{jobs_to_do}]")
...
def _update_gh_statuses(indata: Dict, s3: S3Helper) -> None:
    ...
    ci_cache = CiCache(s3, job_digests).update().fetch_job_statuses()
        ...
        if is_build_job(check_name):
            ...
            if rerun_helper.is_already_finished_by_status():
                ...
                if ci_cache.is_successful(
                    ...
                    job_config.required_on_master,
                ):
                    job_status = ci_cache.pop(
